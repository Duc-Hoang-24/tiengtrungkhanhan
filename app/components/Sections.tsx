...
const menuItems: MenuItem[] = [
  ...
  { label: "Kho Đề Thi", href: "/thu-vien/khodethi" },
];
          ...
          >
            {item.children ? (
              <>
                ...
                >
                  <span 
                  className={`${ isActiveParent (item) ? 'underline underline-offset-4 decoration-1.5 lg:decoration-3': ''}`}
                  >
                  {item.label}
                  </span>
                ...
                {openMenu === item.label && (
                  ...
                  >
                    <div className="w-auto sm:h-30 md:h-40 rounded-md bg-amber-100 flex items-center justify-center flex-col p-2">
                      {item.children.map((child) => (
                        <li key={child.label}>
                          <Link
                            href={child.href}
                            className={`block px-4 py-2 rounded-lg pr-1 w-35 md:w-40 lg:w-50 h-10 hover:bg-amber-200 hover:border-1 border-gray-400 transition ${ isActiveChild(child.href) ? 'bg-amber-300 text-amber-900 font-semibold': ''
                              }`}
                            onClick={() => setOpenMenu(null)}
                          >
                            {child.label}
              ...
              <Link href={item.href!} className={`hover:text-red-300 ${pathname === item.href ? 'underline underline-offset-4 decoration-1.5 lg:decoration-3': ''}`}>
                {item.label}
              </Link>
            )}
          </li>
